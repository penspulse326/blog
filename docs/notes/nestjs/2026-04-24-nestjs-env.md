...
我在使用 Express 做了幾個小專案後感覺到：

1. 除了 middleware 之外幾乎沒有固定的範式

雖然學 Java 通常不會一開始就學 Spring，同樣是先學習怎麼用比較原始的內建函式庫或套件把服務兜出來，了解語言特性與後端流程。

但專案一旦擴大，就很看開發者本身的架構經驗，所以有沒有範式我覺得還是差蠻多，至少學好範式可以養成比較有組織性的撰寫習慣。

2. 週邊套件整合：

Fastify 和 NestJS 整合好許多常用的套件，更新框架的版本時，其依賴套件就會連帶更新，在 Express 就要自己一個一個調了。

更新經常要承擔壞掉的風險，是 JS 生態圈的通病，所以有一個整合好且長期維護的框架會省事很多。

我主要透過線上課程與 NotebookLM 學習 NestJS，因此這邊的練習多以課程內容為主。當然，NestJS 的中文資源，一定會想到那位 XD
...
1. module：各功能的出入口管理
2. controller：MVC 中的 controller
...
應用程式會在 `main.ts` 會透過 `NestFactory.create` 傳入根模組來啟動應用程式：
...
依賴注入是將實例化 (new) 的過程改為在外部完成後，**做為某個類別的建構函式的參數**：
...
實例化會在控制反轉容器 (IoC Container) 裡面完成，確保每個被標記的類別都**只會被實例化一次**，形成單例模式 (Singleton)，並統一管理實例的生命週期與作用域。
...
在官方文件的 **OVERVIEW** 也都單獨條列成一頁，是使用 NestJS 時必須優先知道的功能。

元件可以透過 CLI 快速生成一個有骨架的程式碼：
