...
我在使用 Express 做了幾個小專案有感的幾個麻煩點：

1. 除了 middleware 之外幾乎沒有固定的範式

   > 雖然學 Java 通常不會一開始就學 Spring，同樣是先學習怎麼用比較原始的內建函式庫或套件把服務兜出來，了解語言特性與後端流程。
   >
   > 但專案一旦擴大，就很看開發者本身的架構經驗，所以有沒有範式我覺得還是差蠻多，至少學好範式可以養成比較有組織性的撰寫習慣，雖然寫起來冗長但我認為是必要的 trade-off。
   >
   > 寫作的難度不完全由語言語法的複雜度決定，而是它與你的知識體系產生多少認知負擔。

2. 套件整合：
   > Fastify 和 NestJS 整合好許多常用的套件，更新框架的版本時，其依賴套件就會連帶更新，在 Express 就要自己一個一個調了。更新經常要承擔壞掉的風險，是 JS 生態圈的通病，所以有一個整合好且長期維護的框架會省事很多。
